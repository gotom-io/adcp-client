...
**Targeting `null` is a command, not a value (AdCP 3.2).** On a mutation request every targeting dimension has three states: **omitted** inherits the product default on create / preserves stored state on update, **`null`** suppresses that default / clears the dimension, and a **non-null value** replaces it. Responses, accepted snapshots, and package readback use the strict overlay, whose schema forbids `null`. Persisting a request overlay verbatim writes a clear command into durable state, and echoing it back emits `null` on a shape that rejects it — both silent. Project with `resolveTargetingInput` (create) and `applyTargetingInput` (update) from `@adcp/sdk/server`, or use `createMediaBuyStore`, which does it on both paths. Reject a clear the product cannot execute rather than silently retaining the default. Full table: [`MEDIA-BUY-3.2-COMPATIBILITY.md`](../../docs/guides/MEDIA-BUY-3.2-COMPATIBILITY.md#request-only-targeting-input-null--clear).
